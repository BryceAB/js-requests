...
function changeSayHello() {
  sayHelloButton.classList.add("sayHello");
}
...
// CODE HERE
function changeSayHelloBack() {
  sayHelloButton.classList.remove("sayHello");
}
sayHelloButton.addEventListener("mouseout", changeSayHelloBack);
// PROBLEM 4
/*
...
const ohMy = () => {
  axios.get("http://localhost:3000/animals").then((res) => {
    for (let i = 0; i < res.data.length; i++) {
      let tempdec = document.createElement("p");
      tempdec.textContent = res.data[i];
      document.querySelector("body").appendChild(tempdec);
    }
  });
};
...
document.querySelector("#query-button").addEventListener("click", choiceQuery);
////////////////
//INTERMEDIATE//
////////////////
